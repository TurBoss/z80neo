...
void (* volatile NewFrameFnc)() = NULL; // core 1 remote function

void VgaSetNewFrameCallback(void (*fnc)())
{
  NewFrameFnc = fnc;
}
...
extern "C" void __not_in_flash_func(VgaLine)()
{
	...
	if (line > CurVmode.vtot) { line = 1; if( NewFrameFnc!=NULL ) (*NewFrameFnc)(); }
	int y0;
...
void VgaPioInit()
{
	...
	// SDK 2.x: struct pio_program gained pio_version field; zero-init required
...
void VgaInit(const sVmode* vmode)
{
	...
	// prepare double-width render font pixel mask
	for (i = 0; i < 256; i++)
	{
		u32 m = 0;
		if ((i & B7) != 0) m |= 0xffff;
		if ((i & B6) != 0) m |= 0xffff << 16;
		RenderTextMaskDW[4*i+0] = m;

		m = 0;
		if ((i & B5) != 0) m |= 0xffff;
		if ((i & B4) != 0) m |= 0xffff << 16;
		RenderTextMaskDW[4*i+1] = m;

		m = 0;
		if ((i & B3) != 0) m |= 0xffff;
		if ((i & B2) != 0) m |= 0xffff << 16;
		RenderTextMaskDW[4*i+2] = m;

		m = 0;
		if ((i & B1) != 0) m |= 0xffff;
		if ((i & B0) != 0) m |= 0xffff << 16;
		RenderTextMaskDW[4*i+3] = m;
	}

	// emergency check of structure definitions
	static_assert(SSPRITE_SIZE == sizeof(sSprite), "sSprite size mismatch");
	static_assert(SLAYER_SIZE == sizeof(sLayer), "sLayer size mismatch");
	static_assert(SSEGM_SIZE == sizeof(sSegm), "sSegm size mismatch");
	static_assert(SSTRIP_SIZE == sizeof(sStrip), "sStrip size mismatch");
	static_assert(SSCREEN_SIZE == sizeof(sScreen), "sScreen size mismatch");
...
void VgaCore()
{
	// Initialize multicore lockout for safe flash writes from core 0
