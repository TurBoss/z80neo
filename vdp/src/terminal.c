...
static uint8_t gfx_cmd = 0, gfx_len = 0, gfx_index = 0, gfx_params[16];
static int     gfx_col = 0, gfx_row = 0;
...
// Scroll the bitmap up by one text row and blank the new bottom row.
static void INFLASHFUN terminal_gfx_scroll()
{
  uint8_t chh = font_get_char_height();
  if( chh==0 || (uint32_t) chh > GFX_HEIGHT ) return;
  size_t shift = (size_t) chh * GFX_STRIDE;
  memmove(framebuf_gfx_buf, framebuf_gfx_buf+shift, sizeof(framebuf_gfx_buf)-shift);
  memset(framebuf_gfx_buf + sizeof(framebuf_gfx_buf) - shift, 0, shift);
  gfx_row = (GFX_HEIGHT / chh) - 1;
}


// Render a console character into the bitmap (works while in bitmap mode).
static void INFLASHFUN terminal_gfx_putchar(char c)
{
  uint8_t chh   = font_get_char_height();
  int     ncols = GFX_WIDTH / 8;
  int     nrows = chh ? (GFX_HEIGHT / chh) : 1;

  if( c==13 ) { gfx_col = 0; return; }
  if( c==10 ) { if( ++gfx_row >= nrows ) terminal_gfx_scroll(); return; }
  if( c==8 )  { if( gfx_col>0 ) { gfx_col--; framebuf_gfx_draw_char(gfx_col, gfx_row, ' '); } return; }

  uint8_t u = (uint8_t) c;
  if( u<32 || u==127 ) return;

  framebuf_gfx_draw_char(gfx_col, gfx_row, u);
  if( ++gfx_col >= ncols ) { gfx_col = 0; if( ++gfx_row >= nrows ) terminal_gfx_scroll(); }
}


// Execute a graphics-protocol command whose parameters have been collected
// in gfx_params[].  Protocol: ESC 'g' <cmd> <fixed payload>.
static void INFLASHFUN terminal_gfx_exec()
{
  if( !framebuf_is_dvi() ) return;  // bitmap layer is DVI-only for now

  switch( gfx_cmd )
    {
    case 0x01: // mode: 0 = text, 1 = bitmap
      gfx_col = gfx_row = 0;
      framebuf_gfx_enable(gfx_params[0]!=0);
      break;

    case 0x02: // clear bitmap
      gfx_col = gfx_row = 0;
      framebuf_gfx_clear();
      break;

    case 0x03: // set fg/bg colours
      framebuf_gfx_set_colors(gfx_params[0], gfx_params[1]);
      break;

    case 0x04: // plot pixel
      framebuf_gfx_pixel(gfx_params[0] | (gfx_params[1]<<8),
                         gfx_params[2] | (gfx_params[3]<<8),
                         gfx_params[4]!=0);
      break;

    case 0x05: // draw line
      framebuf_gfx_line(gfx_params[0] | (gfx_params[1]<<8),
                        gfx_params[2] | (gfx_params[3]<<8),
                        gfx_params[4] | (gfx_params[5]<<8),
                        gfx_params[6] | (gfx_params[7]<<8),
                        gfx_params[8]!=0);
      break;

    case 0x06: // fill rectangle
      framebuf_gfx_fill(gfx_params[0] | (gfx_params[1]<<8),
                        gfx_params[2] | (gfx_params[3]<<8),
                        gfx_params[4] | (gfx_params[5]<<8),
                        gfx_params[6] | (gfx_params[7]<<8),
                        gfx_params[8]!=0);
...
void INFLASHFUN terminal_receive_char_vt102(char c)
{
  ...
  if( terminal_state!=TS_NORMAL && terminal_state!=TS_GFXCMD )
    {
      if( c==8 || c==10 || c==13 )
        {
  ...
  // In bitmap mode text is rendered into the bitmap; escapes other than the
  // graphics protocol are ignored.
  if( framebuf_gfx_active && terminal_state==TS_NORMAL )
    {
      if( c==27 ) terminal_state = TS_WAITBRACKET;
      else        terminal_gfx_putchar(c);
      return;
    }
    ...
    case TS_WAITBRACKET:
      {
        ...
        // while the bitmap is displayed, ignore all escapes but graphics
        if( framebuf_gfx_active && c!='g' )
          break;
        ...
        switch( c )
          ...
          case 'g':
            gfx_cmd = 0; gfx_len = 0; gfx_index = 0;
            terminal_state = TS_GFXCMD;
            break;
    ...
    case TS_GFXCMD:
      {
        if( gfx_len==0 )
          {
            // first byte is the command; set the expected payload length
            gfx_cmd = c;
            gfx_index = 0;
            switch( c )
              {
              case 0x01: gfx_len = 1; break;  // mode
              case 0x02: gfx_len = 1; break;  // clear
              case 0x03: gfx_len = 2; break;  // colours
              case 0x04: gfx_len = 5; break;  // pixel
              case 0x05: gfx_len = 9; break;  // line
              case 0x06: gfx_len = 9; break;  // fill
              default:   terminal_state = TS_NORMAL; break;
              }
          }
        else
          {
            gfx_params[gfx_index++] = c;
            if( gfx_index>=gfx_len )
              {
                terminal_gfx_exec();
                terminal_state = TS_NORMAL;
                gfx_len = 0;
